# -*- coding: utf-8 -*-
"""Stocks Watchlist - Yahoo Finance Version"""
...
# Personal Watchlist - Yahoo Finance symbols (add .NS for NSE stocks)
WATCHLIST = {
...
def fetch_stock_data(symbol):
    ...
    try:
        # Get historical data for last 30 days with hourly intervals
        ...
        # yfinance already provides the right column names
        # Ensure we have the required columns
        required_cols = ['Open', 'High', 'Low', 'Close']
        if not all(col in data.columns for col in required_cols):
            st.warning(f"Missing required columns for {symbol}. Available: {data.columns.tolist()}")
            return None
...
def get_quote_data(symbol):
    """Get full quote including change, volume etc using yfinance"""
    try:
        ticker = yf.Ticker(symbol)
        info = ticker.info
        ...
        return {
            ...
            'change': change,
            'pChange': pct_change,
            'open': info.get('open', info.get('regularMarketOpen', today_data['Open'].iloc[0])),
            'dayHigh': info.get('dayHigh', info.get('regularMarketDayHigh', today_data['High'].max())),
            'dayLow': info.get('dayLow', info.get('regularMarketDayLow', today_data['Low'].min())),
    ...
    for result in valid_results:
        ...
        for i in range(len(prices)):
            ...
            if price is not None and change is not None:
                ...
                html += f'<td style="background-color: #1e1e1e" title="Price: ₹{price:.2f}">₹{price:.0f}</td>'
            else:
                html += '<td style="background-color: #1e1e1e">-</td>'
        html += '</tr>'
...
def main():
    ...
    st.markdown(f"**Live Data via Yahoo Finance** | Last Updated: {datetime.now().strftime('%H:%M:%S')}")
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.caption("💡 **Strategy:** Look for RSI divergence + Williams %R extreme zones")
        st.caption("📊 **Data:** Real-time from Yahoo Finance")
    ...
    # Main tabs
    tab1, tab2, tab3 = st.tabs(["📊 Overview", "🎯 Signals", "🔥 Heatmap"])
    ...
    with tab1:
        ...
        col3.metric("⚡ Alerts", wr_alerts)
        col4.metric("📊 Stocks", len(results))

        st.markdown("---")

        # Display each stock
        for result in results:
            with st.expander(f"**{result['symbol']}** - ₹{result['price']:.2f}", expanded=False):
                col1, col2, col3, col4 = st.columns(4)
                ...
                with col1:
                    ...
                    st.caption(f"High: ₹{result['day_high']:.2f} | Low: ₹{result['day_low']:.2f}")

                with col2:
                    if result['rsi'] is not None:
                        rsi_emoji = "🟢" if result['rsi'] < 40 else "🔴" if result['rsi'] > 60 else "⚪"
                        st.metric("RSI", f"{result['rsi']:.1f} {rsi_emoji}", result['rsi_signal'])
                        if result['rsi_signal'] not in ['NEUTRAL', 'NO DATA']:
                            st.caption(f"💪 Strength: {result['rsi_strength']:.0f}/100")
                    else:
                        st.metric("RSI", "N/A", "NO DATA")

                with col3:
                    if result['williams_r'] is not None:
                        wr_emoji = "🔴" if "OVERBOUGHT" in result['wr_zone'] else "🟢" if "OVERSOLD" in result['wr_zone'] else "⚪"
    ...
    with tab2:
        ...
        if not signal_results:
            st.info("😴 No active signals. Market is in neutral zone.")
        else:
            # RSI Signals
            st.markdown("### 📊 RSI Divergence Signals")

            col1, col2 = st.columns(2)

            with col1:
                st.markdown("**🟢 BULLISH DIVERGENCE**")
            ...
            with col2:
                ...
                if bearish:
                    ...
                    for stock in bearish:
                        st.error(f"**{stock['symbol']}** - ₹{stock['price']:.2f}")
                        st.caption(f"RSI: {stock['rsi']:.1f} | Strength: {stock['rsi_strength']:.0f}/100")
                else:
            ...
            # Williams %R Alerts
            st.markdown("### ⚡ Williams %R Extreme Zones")
            ...
            if wr_stocks:
                ...
                with col1:
                    ...
                    if ob:
                        for stock in ob:
                            st.warning(f"**{stock['symbol']}** - ₹{stock['price']:.2f}")
